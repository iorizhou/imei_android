...
import com.imei666.android.BuildConfig;
...
public class FileOpenIntentUtil {
    ...
    public static void openFile(String filepath){
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
            ...
            uri = FileProvider.getUriForFile(ImeiApplication.getInstace(), BuildConfig.APPLICATION_ID + ".fileProvider", new File(filepath));
        }else {
            uri =  Uri.fromFile(new File(filepath ));
        }
        intent.setDataAndType(uri, getContentType(filepath));
        ImeiApplication.getInstace().startActivity(intent);
    }
