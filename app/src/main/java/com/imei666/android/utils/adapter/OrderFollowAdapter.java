...
public class OrderFollowAdapter extends BaseAdapter {
    ...
    private List<OrderFollow> traceList = new ArrayList<>(1);
    private static final int TYPE_TOP = 0x0000;
    private static final int TYPE_NORMAL= 0x0001;

    public OrderFollowAdapter(Context context, List<OrderFollow> traceList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        final OrderFollow trace = getItem(position);
        if (convertView != null) {
            holder = (ViewHolder) convertView.getTag();
        } else {
            holder = new ViewHolder();
            convertView = LayoutInflater.from(context).inflate(R.layout.item_orderdetail_follow, parent, false);
